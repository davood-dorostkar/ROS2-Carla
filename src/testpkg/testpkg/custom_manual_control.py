...
class ManualControl(CompatibleNode):
    ...
    def __init__(self, resolution):
        ...
        self.image_subscriber = self.new_subscription(
            Image, "/carla/{}/rgb_view/image".format(self.role_name), self.on_view_image, qos_profile=10
        )

        self.collision_subscriber = self.new_subscription(
            CarlaCollisionEvent, "/carla/{}/collision".format(self.role_name), self.on_collision, qos_profile=10
        )
...
class KeyboardControl(object):
    ...
    def __init__(self, role_name, hud, node):
        ...
        self.vehicle_control_manual_override_publisher = self.node.new_publisher(
            Bool, "/carla/{}/vehicle_control_manual_override".format(self.role_name), qos_profile=fast_latched_qos
        )

        self.vehicle_control_manual_override = False

        self.auto_pilot_enable_publisher = self.node.new_publisher(
            Bool, "/carla/{}/enable_autopilot".format(self.role_name), qos_profile=fast_qos
        )

        self.vehicle_control_publisher = self.node.new_publisher(
            CarlaEgoVehicleControl, "/carla/{}/vehicle_control_cmd_manual".format(self.role_name), qos_profile=fast_qos
        )
...
class HUD(object):
    ...
    def __init__(self, role_name, width, height, node):
        ...
        self._font_mono = pygame.font.Font(mono, 11)
        ...
        self.gnss_subscriber = node.new_subscription(
            NavSatFix, "/carla/{}/gnss".format(self.role_name), self.gnss_updated, qos_profile=10
        )

        self.odometry_subscriber = node.new_subscription(
            Odometry, "/carla/{}/odometry".format(self.role_name), self.odometry_updated, qos_profile=10
        )
    ...
    def update_info_text(self):
        ...
        self._info_text += [
            ...
            "Gear:        %s"
            % {-1: "R", 0: "N"}.get(self.vehicle_status.control.gear, self.vehicle_status.control.gear),
            "",
        ]
...
def main(args=None):
    ...
    try:
        display = pygame.display.set_mode(
            (resolution["width"], resolution["height"]), pygame.HWSURFACE | pygame.DOUBLEBUF
        )
