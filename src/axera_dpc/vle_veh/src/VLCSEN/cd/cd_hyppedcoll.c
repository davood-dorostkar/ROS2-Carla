...
typedef struct CDHistoryFloat {
    ...
    float32 afValues[CD_PED_COLL_OBSERVE_Y_DIST_N];
} CDHistoryFloat_t;
...
/*! @brief      Target object kinematic history structure
   ...
   for Pedestrian Collision Hypothesis
...
/* ****************************************************************
    TYPEDEF
    **************************************************************** */
/*! @brief     Pedestrian Collision Hypothesis object history list

    @general   List to store the object history of the total
   objects(Envm_N_OBJECTS) used for Pedestrian Collision Hypothesis

    */
typedef CDKinHistObj_t CDPedHypInternalObjHistList_t[Envm_N_OBJECTS];
...
/*! @brief      Maximum weight of measured pedestrian velocity added to velocity
...
static CDPedHypInternalObjHistList_t CDPedHypInternalObjHistList;
static CDKinHistEgo_t CDPedHypInternalEgoHist;
...
static void CDHypoPedCollCalculatePSCProb(
    ObjNumber_t iObjectIndex,
    const CDInternalStatus_t *pInternalStatus,
    CDInternalObject_t *pCurrentObjIntern,
...
static void CDHypoPedUpdateObjHist(ObjNumber_t iObjectIndex,
                                   const CDObjectData_t *pObjectData);
...
static void CDHypoPedUpdateEgoHist(const EMPKinEgo_t *pEgoKin);
...
static void CDPedCollDowngradeProbDominantVelo(
...
/*************************************************************************************************************************
  Functionname:    CDHypoPedCollObjectFilter */ /*!
                                                                           ...
                                                                           objects
                                                                           for
                                                                                              calculation of Pedestrian Collision Hypothesis
...
/* ***********************************************************************
  @fn            CDHypoPedCollCalculateProb */ /*!

                              @brief         Calculates the Pedestrian Collision
                            Hypothesis Probability

                              @description   This function calculates the
                            Pedestrian Collision Hypothesis Probability by
...
static void CDHypoPedCollCalculateProb(
    ...
    float32 fObjCollProb = 1.0f; /*i.e., EMPCalcCollProbPed probability*/

    {
        ...
        /********************************************/
        CDHypoPedCollCalculatePSCProb(iObjectIndex, pInternalStatus,
                                      pCurrentObjIntern, fObjDistToEgoTraj_m,
                                      fObjYProjLength_m, fEgoVeloDepWidth_m,
                                      &fPSCProb);
    }
...

    {
        ...
        CDPedCollDowngradeProbDominantVelo(
        ...
        pCurrentObjIntern->sHypPedCollData.fDowngradeFactor = out_fDownGradeFac;
    }
...
/* ***********************************************************************
  @fn            CDPedCollDowngradeProbDominantVelo */ /*!

                      @brief         Downgrade the Pedestrian Collision
                    Probability

                      @description   The function downgrade the Pedestrian
...
static void CDPedCollDowngradeProbDominantVelo(
    ...
    /*
    Thoughts:
    Downgrading the pedestrian collision probability is a workaround, to use
    ...
    */
    const float32 fPedDistToVehiclePath_m =
    ...
    const float32 fMaxRate = CML_f_CalculatePolygonValue(
        ...
        CD_PED_COLL_DOMINANT_DOWNGRADE_MAX_RATE_VEC, fPedDistToVehiclePath_m);
    const float32 fMinFactor = CML_f_CalculatePolygonValue(
        CD_NUMBER_OF_DOMINANTDOWNGRADE_MIN_FACTOR_VALUES,
        CD_PED_COLL_DOMINANT_DOWNGRADE_MIN_FACTOR_VEC, fPedDistToVehiclePath_m);
    ...
    const boolean bPedIsMoving = (boolean)(
        fObjVelo_mps > CD_PED_COLL_DOMINANT_DOWNGRADE_MIN_OBJ_VELO_MPS);

    const boolean bPedIsNotInVehiclePath =
    ...
    *out_fDownGradeFac = 1.0f;
    ...
    if (fABS(fObjVrelOnEgoTraj_mps) > C_F32_DELTA) {
        ...
        if ((FALSE != bPedIsMoving) && (FALSE != bPedIsNotInVehiclePath)) {
...
/* ***********************************************************************
  @fn            CDHypoPedCollCalculatePSCProb */ /*!

                           @brief         Pedestrian Collision Hypothesis
                         calculation with the Probability of Speed Concept

                                          1. Calculation of the speed range, the
                         pedestrian has to walk for a collision
                                          2. Calculation of the probability that
                         the pedestrian walks a certain speed (Probability
                         Density)
...
static void CDHypoPedCollCalculatePSCProb(
    ...
    float32 afPossibleVelocity_mps[CD_PED_COLL_PED_VELO_STEPS_N];
    float32 afVelocityProbabilities[CD_PED_COLL_PED_VELO_STEPS_N];

    uint8 uiNumberOfValidDataPoints;
    float32 afDistToTrajHistory[CD_PED_COLL_OBSERVE_Y_DIST_N];
    float32 afDistOnTrajHistory[CD_PED_COLL_OBSERVE_Y_DIST_N];
    float32 afDistToTrajVarHistory[CD_PED_COLL_OBSERVE_Y_DIST_N];
    float32 afYDistVarHistory[CD_PED_COLL_OBSERVE_Y_DIST_N];
    float32 afTimeVec_s[CD_PED_COLL_OBSERVE_Y_DIST_N];

    float32 fNormFac;

    const float32 fPedVeloToTraj =
        pCurrentObjIntern->TrajRelation.fVelocityToTraj;
    const float32 fPedVeloToTrajStd =
    ...
    if (pCurrentObjIntern->TTC3 > C_F32_DELTA) {
        ...
        fCollCorrVeloLeft_mps =
            -((fObjDistToEgoTraj_m - (fObjYProjLength_m / 2.0f)) -
              ((fEgoWidth_m * PED_VELO_VEHICLE_PATH_FACTOR) / 2.0f)) /
    ...
    fStepSizePossibleVelocity_mps =
        (CD_PED_COLL_PED_VELO_MAX_MPS - CD_PED_COLL_PED_VELO_MIN_MPS) /
        (float32)(CD_PED_COLL_PED_VELO_STEPS_N - 1u);
    for (i = 0; i < CD_PED_COLL_PED_VELO_STEPS_N; ++i) {
        afPossibleVelocity_mps[i] = (CD_PED_COLL_PED_VELO_MIN_MPS +
    ...
    */
    ...
    uiNumberOfValidDataPoints = CDGetCompleteHistory(
        &(CDPedHypInternalObjHistList[iObjectIndex].sPosYVar),
        afYDistVarHistory, (uint8)CD_PED_COLL_OBSERVE_Y_DIST_N);

    (void)CDCalcHistDistToTraj(&((*pInternalStatus).sKinEgo),
                               &(CDPedHypInternalObjHistList[iObjectIndex]),
                               afDistToTrajHistory, afDistOnTrajHistory);

    for (i = 0; i < CD_PED_COLL_OBSERVE_Y_DIST_N; ++i) {
    ...
    if ((FALSE !=
         HistIntegrate(afPossibleVelocity_mps, afVelocityProbabilities,
                       afPossibleVelocity_mps[0],
                       afPossibleVelocity_mps[CD_PED_COLL_PED_VELO_STEPS_N - 1],
                       CD_PED_COLL_PED_VELO_STEPS_N, &(fNormFac))) &&
    ...
    pCurrentObjIntern->sHypPedCollData.fCollCorrVeloLeft_mps =
        fCollCorrVeloLeft_mps;
    pCurrentObjIntern->sHypPedCollData.fCollCorrVeloRight_mps =
        fCollCorrVeloRight_mps;
    for (i = 0; i < CD_PED_COLL_PED_VELO_STEPS_N; ++i) {
        pCurrentObjIntern->sHypPedCollData
    ...
    if (FALSE !=
        HistIntegrate(
            ...
            CD_PED_COLL_PED_VELO_STEPS_N,
            &(pCurrentObjIntern->sHypPedCollData.fHypProbOnlyFitting))) {
        /*the upper limit of probabilities is 1*/

        pCurrentObjIntern->sHypPedCollData.fHypProbOnlyFitting = MIN_FLOAT(
            1.0f, (pCurrentObjIntern->sHypPedCollData.fHypProbOnlyFitting));
    }
     ...
     * probability dense function */
    {
        const float32 fWeight =
            ...
            PED_VELO_ADD_MEASURED_VELO_MAX_WEIGHT;
        float32 afVelocityMeasuredGaussian[CD_PED_COLL_PED_VELO_STEPS_N];
        ...
        for (i = 0; i < CD_PED_COLL_PED_VELO_STEPS_N; ++i) {
            ...
            const float32 fGaussCDFLeft = BML_f_CalcStdGaussianCDF(
                ...
                fPedVeloToTraj, fPedVeloToTrajStd);
        ...
        if (FALSE !=
            HistIntegrate(
                ...
                fCollCorrVeloLeft_mps, fCollCorrVeloRight_mps,
                CD_PED_COLL_PED_VELO_STEPS_N,
                &(pCurrentObjIntern->sHypPedCollData.fHypProbOnlyGaussian))) {
            pCurrentObjIntern->sHypPedCollData.fHypProbOnlyGaussian = MIN_FLOAT(
                1.0f,
                (pCurrentObjIntern->sHypPedCollData.fHypProbOnlyGaussian));
        }
    }
    ...
    if (FALSE != HistIntegrate(afPossibleVelocity_mps, afVelocityProbabilities,
                               fCollCorrVeloLeft_mps, fCollCorrVeloRight_mps,
                               CD_PED_COLL_PED_VELO_STEPS_N, pOut_fPSCProb)) {
        *pOut_fPSCProb = MIN_FLOAT(1.0f, *pOut_fPSCProb);
    }
    /* Add Data to CDInternalObject for Measefreeze */
    pCurrentObjIntern->sHypPedCollData.fHypProbPlusGaussian = *pOut_fPSCProb;
...
static void CDPushHistoryFloat(float32 fLatestValue,
    ...
    uint8 uiNextID =
        (pIn_FloatHistory->uiLastID + 1u) % CD_PED_COLL_OBSERVE_Y_DIST_N;
    /* Update LastID */
...
static void CDResetHistoryFloat(CDHistoryFloat_t *pIn_FloatHistory) {
    ...
    for (uiCurrID = 0; uiCurrID < CD_PED_COLL_OBSERVE_Y_DIST_N; ++uiCurrID) {
...
/* ***********************************************************************
  @fn            CDGetNthHistoryFloat  */ /*!
                                   ...
                                   @brief         Returns the Nth-History Float.
                                                  If steps >
                                 CD_PED_COLL_OBSERVE_Y_DIST_N it returns the
...
static boolean CDGetNthHistoryFloat(uint8 uiSteps,
    ...
    /* Wrap around */
    if (iShiftedID < 0) {
        uiTargetID = (uint8)(CD_PED_COLL_OBSERVE_Y_DIST_N - (-iShiftedID));
    } else {
...
static boolean CDCalcHistDistToTraj(const EMPKinEgo_t *pIn_KinHistEgo,
    ...
    for (uiCurrTimeStep = 0; uiCurrTimeStep < CD_PED_COLL_OBSERVE_Y_DIST_N;
         ++uiCurrTimeStep) {
...
/* ***********************************************************************
  @fn            CDHypoPedUpdateEgoHist */ /*!
                                ...
                                */
static void CDHypoPedUpdateEgoHist(const EMPKinEgo_t *pEgoKin) {
    CDPushHistoryFloat(pEgoKin->fAccel, &(CDPedHypInternalEgoHist.sAccel));
    CDPushHistoryFloat(pEgoKin->fVel, &(CDPedHypInternalEgoHist.sVel));
    CDPushHistoryFloat(pEgoKin->fYawRate, &(CDPedHypInternalEgoHist.sYawRate));
}
...
/* ***********************************************************************
  @fn            CDHypoPedCollMain */ /*!

                                       @brief         Handles the Pedestrian
                                     ...
                                     ****************************************************************************
                                     */
void CDHypoPedCollMain(ObjNumber_t iObjectIndex,
    ...
    CD_CLEAR_HYP_BIT(&(pLocalObject->bitHypPresel),
                     (uint32)CDHypothesisType_PedCollision);
    CD_CLEAR_HYP_BIT(&(pLocalObject->bitHypActive),
                     (uint32)CDHypothesisType_PedCollision);

    /* Update internal object history (only needed for PSC calculation) */
    CDHypoPedUpdateObjHist(iObjectIndex, (pInputData->pObjectData));
    CDHypoPedUpdateEgoHist(&(pInternalStatus->sKinEgo));

    /* Default Hypothesis Prob Calculation calling */
    if (bObjFilterMatched != FALSE) {
        VLCSEN_SERVICE_ADD_EVENT(
            e_RTA_EVT_AlgoStart, VLCSEN_RTA_CD_HYP_PEDCOLL_SINGLE,
            (uint8)(iObjectIndex)); /* start profiling for Hypothesis */
        if (CDHypoPedCollObjectFilter(iObjectIndex, pInputData->pObjectData,
                                      pLocalObject) != FALSE) {
            ...
            /* set hypothesis type */
            Hypothesis.eType = CDHypothesisType_PedCollision;

            CDHypoPedCollCalculateProb(iObjectIndex, &Hypothesis, pInputData,
                                       pInternalStatus, pLocalObject);
...
static void iterateAllPossVelo(const uint8 uiNumberOfValidDataPoints,
                               ...
                               float32 afTimeVec_s[],
                               float32 afVelocityProbabilities[]) {
    for (int i = 0; i < CD_PED_COLL_PED_VELO_STEPS_N; ++i) {
        float32 afWeights[CD_PED_COLL_OBSERVE_Y_DIST_N];
        float32 fWeightSum = 0;
        float32 fMaxWeight = 0;
        float32 fBWeighted =
            0; /* Best linear fitted function: y=ped_velo_mps*t+fBWeighted */
        float32 fError = 0;
        uint8 j;

        /* Calculate Weights for y distance history */
        for (j = 0; j < CD_PED_COLL_OBSERVE_Y_DIST_N; ++j) {
            if (((CD_PED_COLL_OBSERVE_Y_DIST_N - j) - 1) <
                uiNumberOfValidDataPoints) {
                float32 fWeightStdPart =
                    1.0f / (SQRT(ABS(afDistToTrajVarHistory[(
                                (CD_PED_COLL_OBSERVE_Y_DIST_N - j) - 1)])) +
                            PED_VELO_WEIGHT_STD_B);
                float32 fWeightLinearPart =
                    (((PED_VELO_WEIGHT_YOUNGEST - PED_VELO_WEIGHT_OLDEST) /
                      (CD_PED_COLL_OBSERVE_Y_DIST_N - 1)) *
                     j) +
                    PED_VELO_WEIGHT_OLDEST;
            ...
            afTimeVec_s[j] = -(
                (((float32)CD_PED_COLL_OBSERVE_Y_DIST_N - (float32)j) - 1.0f) *
                VLC_CYCLE_TIME);
        }
        ...
        for (j = 0; j < CD_PED_COLL_OBSERVE_Y_DIST_N; ++j) {
            if (((CD_PED_COLL_OBSERVE_Y_DIST_N - j) - 1) <
                ...
                ped_velo_mps+fStepSizePossibleVelocity_mps/2)
                - Constant offset
                */
                float32 t = afTimeVec_s[j];
                ...
                float32 fAllowedError = fAllowedErrorDiscretization +
                                        PED_VELO_ALLOWED_CONST_ERROR_M;
                ...
                fError += (fErrorAtThisPoint * afWeights[j]) /
                          (CD_PED_COLL_OBSERVE_Y_DIST_N * fMaxWeight);
            }
        }
        /* Map Error to Probability */
        afVelocityProbabilities[i] =
            PED_VELO_MAP_ERROR_PROB_CONST_A /
            (PED_VELO_MAP_ERROR_PROB_CONST_B + (fError * fError * fError));
    }
}
