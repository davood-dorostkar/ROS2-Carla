...
class Obstacle {
   public:
    ...
    void Show_ob_prediction(geometry_msgs::msg::Pose &obstacle_trajectory_, const TrajectoryPoint trajectory_point_); //use
    void visualization(const std::vector<Obstacle> Obstacles_);

    const Prediction::Ob_Trajectory &Trajectory() const { return trajectory_; } //use

    void SetTrajectory(const Prediction::Ob_Trajectory trajectory) { trajectory_ = trajectory; } //use

    Obstacle_avoid *const GetOba() { return &oba; } //use
