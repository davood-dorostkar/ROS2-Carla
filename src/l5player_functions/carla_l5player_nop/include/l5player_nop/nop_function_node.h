...
public:
    ...
    void ObstacleWrapperForRviz(visualization_msgs::msg::Marker &marker, const Obstacle *Primitive_obstacle,
                                     const int id);
...
public:
    ...
    Eigen::MatrixXd routing_waypoints_; // 导航路径的中心点坐标，原始输入
...
private:
    std::unique_ptr<std::thread> reference_line_thread_{nullptr};
