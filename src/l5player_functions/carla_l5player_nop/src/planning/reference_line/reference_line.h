...
class referenceLine {
   ...
   public:
    ...
    void process(Eigen::MatrixXd& hdmap_way_points);
    ...
    const std::vector<ReferencePoint>* const GetReferencePoints() {
        std::cout << "guard GetReferencePoints in!!!" << std::endl;
        std::lock_guard<std::mutex> guard(mutex_);
        std::cout << "guard GetReferencePoints out!!!" << std::endl;
   ...
   public:
    ...
    // param
    Eigen::MatrixXd routing_waypoints_;                 // 中心点坐标, original input
    ...
    std::vector<ReferencePoint> reference_points;    // 参考路径点参数, final output for plan
