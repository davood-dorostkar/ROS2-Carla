...
**************************************************************************************'''*/
{
    ...
    reference_line_thread_ = std::make_unique<std::thread>([this]() { rl_.process(routing_waypoints_); });
    // rl_.process(routing_waypoints_);
    ...
    global_path_publish_timer =
        this->create_wall_timer(5000ms, std::bind(&NopFunctionNode::GlobalPathPublishCallback, this));
...
void NopFunctionNode::NopRunOnce() {
    ...
    std::lock_guard<std::mutex> guard(obstacle_mutex_);
    ...
    if (obstacles.size() > 0) {
        ...
        for (int i = 0; i < obstacles.size(); ++i) {
            ...
            ObstacleWrapperForRviz(marker, obstacles[i], i);
    ...
    std::cout << "reference_points_ptr->size: " << reference_points_ptr->size() << std::endl;
...
void NopFunctionNode::SetObjectArrayCallback(derived_object_msgs::msg::ObjectArray::SharedPtr msg) {
    ...
    if (objects.size() > (size_t)0) {
        for (size_t i = 0; i < objects.size(); ++i) {
            ...
            {
                Ob_Trajectory ob_tray =
                    obs.Generater_Trajectory(obs.centerpoint, 4, obs.obstacle_theta, obs.obstacle_velocity);
                obs.SetTrajectory(ob_tray);
            }

            /*---------------------------------------过滤不在有效区域的障碍物-------------------------------------------*/
            // PPoint centerpoint(obs.centerpoint.position.x, obs.centerpoint.position.y);
            // if (obs.Inside_rectangle(front_left, back_left, back_right, front_right, centerpoint) == false)
            // {
            ...
            } else if (obs.obstacle_type == 4)    // 圆形：Pedestrian
            {
                ...
                obs.CalculateCarBoundaryPoint(1, 1, ob_center, obs.obstacle_theta, ob_left_front, ob_left_buttom,
...
**************************************************************************************'''*/
{
    ...
    // publish for orininal points for rviz
    global_path.poses.clear();
    for (Eigen::Index i = 0; i < routing_waypoints_.rows(); ++i) {
        this_pose_stamped.header.frame_id = "gps";
        this_pose_stamped.header.stamp = this->get_clock()->now();
        this_pose_stamped.pose.position.x = routing_waypoints_(i, 0);
        this_pose_stamped.pose.position.y = routing_waypoints_(i, 1);
        this_pose_stamped.pose.position.z = 0;
        this_pose_stamped.pose.orientation.x = 0;
        this_pose_stamped.pose.orientation.y = 0;
        this_pose_stamped.pose.orientation.z = 0;
...
void NopFunctionNode::ObstacleWrapperForRviz(visualization_msgs::msg::Marker &marker,
