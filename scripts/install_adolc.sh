...
sudo apt-get update && apt-get install -y autoconf automake libtool
...
wget https://www.coin-or.org/download/source/ADOL-C/ADOL-C-2.6.3.zip 
unzip ADOL-C-2.6.3.zip

pushd ADOL-C-2.6.3
sudo autoreconf --install
sudo automake
sudo ./configure --prefix="$PWD" --enable-sparse --enable-addexa --with-openmp-flag="-fopenmp" --with-colpack="/usr/local/colpack" ADD_CXXFLAGS="-fPIC" ADD_CFLAGS="-fPIC" ADD_FFLAGS="-fPIC"
